...
@onready var mat_glow = $Guns/Gun1.get_surface_override_material(1)
...
func _process(delta):
	# fade to new color
	mat_glow.emission = mat_glow.emission.lerp(Enums.get_bullet_color(current_color), 0.1)
	mat_glow.emission_energy = 0.8+0.5*pow(0.5+0.5*sin(Time.get_ticks_msec() / 500.0),7)
...
func _physics_process(delta):
	...
	for g in $Guns.get_children():
		g.position *= clamp(1 - delta * 10, 0.0, 1.0)
		g.position -= (velocity) * g.global_transform.basis * delta * 5
		
		# also add walk jiggle
		if should_walk_jiggle:
			g.position -= clamp(velocity_xz.length(), 0.0, 1.0) * Vector3(0,1,0) * sin(accumulated_movement * 1.7) * delta * 2
